...
VulkanGraphicsProgram::VulkanGraphicsProgram(
	...
	m_shaderStageCreateInfos = std::move(compiled.stagesCreateInfo);

	Init(compiled.bindingsInfo);

	CreatePipeline(m_defaultRenderPass);
}

VulkanGraphicsProgram::~VulkanGraphicsProgram() {
	VkDevice device = m_device.GetDevice();

	for (auto& [renderPass, pipeline] : m_pipelines) {
...
VkPipeline VulkanGraphicsProgram::GetOrCreatePipeline(VkRenderPass renderPass) {
	auto it = m_pipelines.find(renderPass);
	if (it != m_pipelines.end()) {
		return it->second;
	}
	CreatePipeline(renderPass);
	return m_pipelines[renderPass];
}

void VulkanGraphicsProgram::CreatePipeline(VkRenderPass renderPass) {
	VkDevice device = m_device.GetDevice();

	if (m_shaderStageCreateInfos.empty()) {
		throw std::runtime_error("No shader stages available for pipeline creation");
	}

	auto bindingDescriptions = VulkanMesh::GetBindingDescriptions();
	...
	pipelineInfo.stageCount = static_cast<uint32_t>(m_shaderStageCreateInfos.size());
	...
	if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) !=
	    VK_SUCCESS) {
		throw std::runtime_error("Failed to create graphics pipeline for render pass");
	}
